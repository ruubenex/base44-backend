...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-white">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <div className="text-left">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }} className="inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-xl border border-orange-200 rounded-full shadow-lg shadow-orange-100 mb-8">
              ...
              <span className="text-sm font-medium text-gray-700">{h.badge}</span>
            </motion.div>

            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-gray-900 mb-6 tracking-tight">
              {h.headline1}{' '}
              <span className="relative inline-block">
                ...
                <motion.span initial={{ width: 0 }} animate={{ width: '100%' }} transition={{ delay: 0.8, duration: 0.8 }} className="absolute bottom-2 left-0 h-3 bg-orange-200/50 -z-0 rounded-full" />
              </span>
            </motion.h1>

            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.6 }} className="text-lg md:text-xl text-gray-600 max-w-xl mb-10 leading-relaxed">
              {h.description}
              <span className="font-semibold text-gray-800">{h.descriptionBold}</span>
              {h.descriptionEnd}
            </motion.p>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.6 }} className="flex flex-col sm:flex-row items-start gap-4">
              <a href="https://camcal.pro/" target="_blank" rel="noopener noreferrer">
                <Button size="lg" className="group bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-8 py-6 text-lg rounded-2xl shadow-xl shadow-orange-500/30 hover:shadow-2xl hover:shadow-orange-500/40 transition-all duration-300 hover:scale-105">
                  <Camera className="w-5 h-5 mr-2" />
                  {h.cta}
                  <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
                </Button>
              </a>
              <Button variant="ghost" size="lg" className="px-6 py-6 text-lg text-gray-600 hover:text-orange-600 transition-all duration-300 gap-2" onClick={() => document.getElementById('features').scrollIntoView({ behavior: 'smooth' })}>
            ...
            {/* User Avatars + Rating */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.6 }} className="mt-10 flex items-center gap-4">
              <div className="flex -space-x-3">
                {userAvatars.map((avatar, i) => (
                  <img key={i} src={avatar} alt="" className="w-10 h-10 rounded-full border-2 border-white object-cover shadow-sm" loading="lazy" />
                ))}
              </div>
              <div>
                ...
                <p className="text-sm text-gray-500">50.000+ {h.activeUsers || 'usuários ativos'}</p>
              </div>
            </motion.div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.8 }} className="relative flex justify-center lg:justify-end">
            <div className="relative mx-auto max-w-[280px] sm:max-w-[320px] md:max-w-[360px]">
              ...
              <div className="absolute -left-4 sm:-left-16 md:-left-20 top-1/4 bg-white/90 backdrop-blur-sm rounded-xl md:rounded-2xl p-2 sm:p-3 md:p-4 shadow-lg border border-white/50 animate-bounce" style={{ animationDuration: '3s' }}>
              ...
              <div className="absolute -right-4 sm:-right-16 md:-right-20 top-1/2 bg-white/90 backdrop-blur-sm rounded-xl md:rounded-2xl p-2 sm:p-3 md:p-4 shadow-lg border border-white/50 animate-bounce" style={{ animationDuration: '3.5s', animationDelay: '0.5s' }}>
                <div className="flex items-center gap-2 md:gap-3">
      ...
      {/* Scroll Indicator */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.6 }} className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
        <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} className="flex flex-col items-center text-gray-400">
          <span className="text-xs uppercase tracking-widest mb-2">{h.scrollHint}</span>
          <div className="w-6 h-10 border-2 border-gray-300 rounded-full flex justify-center pt-2">
            <motion.div animate={{ y: [0, 12, 0] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} className="w-1.5 h-3 bg-orange-500 rounded-full" />
          </div>
        </motion.div>
      </motion.div>
    </section>
