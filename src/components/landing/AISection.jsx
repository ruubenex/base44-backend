...
export default function AISection({ isDarkMode }) {
  ...
  const dietTags = [
    { emoji: '🌱', label: a.dietTags.vegan },
    { emoji: '🥑', label: a.dietTags.keto },
    { emoji: '🌾', label: a.dietTags.glutenFree },
    { emoji: '🥛', label: a.dietTags.lactoseFree },
    { emoji: '🥩', label: a.dietTags.carnivore },
    { emoji: '✂️', label: a.dietTags.cutting },
  ];

  return (
    <section ref={sectionRef} className={`py-16 md:py-32 relative overflow-hidden transition-colors duration-1000 ${isDarkMode ? 'bg-[#0A0A0A]' : 'bg-white'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}>
            ...
            <p className={`text-base md:text-lg mb-6 md:mb-8 leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{a.description}</p>

            {/* Diet Tags */}
            <motion.div initial={{ opacity: 0, y: 15 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4, duration: 0.5 }} className="flex flex-wrap gap-2 mb-6 md:mb-8">
              {dietTags.map((tag, i) => (
                <span key={i} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs md:text-sm font-medium ${isDarkMode ? 'bg-white/5 text-gray-300 border border-white/10' : 'bg-gray-50 text-gray-700 border border-gray-200'}`}>
                  {tag.emoji} {tag.label}
                </span>
              ))}
            </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {a.items.map((feature, index) => {
                ...
                return (
                  <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.3 + index * 0.1, duration: 0.5 }} className={`flex items-start gap-3 p-3 md:p-4 rounded-2xl transition-all duration-300 hover:scale-105 ${isDarkMode ? 'bg-white/5 hover:bg-white/10' : 'bg-gray-50 hover:bg-orange-50'}`}>
                    ...
                    <div className="min-w-0">
                      <h4 className={`font-semibold mb-1 text-sm md:text-base ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{feature.title}</h4>
                      <p className={`text-xs md:text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{feature.description}</p>
                    </div>
                  </motion.div>
                );
              })}
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="relative mt-8 lg:mt-0">
            <div className="relative mx-auto max-w-[240px] sm:max-w-xs">
              ...
              <div className={`absolute -top-2 -right-2 md:-top-4 md:-right-4 px-3 py-2 md:px-4 md:py-3 rounded-xl md:rounded-2xl shadow-lg animate-bounce ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`} style={{ animationDuration: '3s' }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs md:text-base font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{a.aiActive}</span>
                </div>
              </div>
              <div className={`absolute -bottom-2 -left-2 md:-bottom-4 md:-left-4 px-3 py-2 md:px-4 md:py-3 rounded-xl md:rounded-2xl shadow-lg animate-bounce ${isDarkMode ? 'bg-gradient-to-r from-orange-600 to-red-600' : 'bg-gradient-to-r from-orange-500 to-red-500'}`} style={{ animationDuration: '3.5s', animationDelay: '0.5s' }}>
                <div className="flex items-center gap-2 text-white">
                  ...
                  <span className="text-xs md:text-base font-semibold">{a.plan40days}</span>
                </div>
              </div>
            </div>
          </motion.div>
